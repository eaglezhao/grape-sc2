            ...
            if (Config.OutputErrors) {
                ...
                if (s != null) {
                    if (!s.IsLogicalChildOfEntityType<GrapeWhileStatement>() || !s.IsLogicalChildOfEntityType<GrapeForEachStatement>()) {
                        errorSink.AddError(new GrapeErrorSink.Error { Description = "A continue statement must be the logical child of a while or foreach statement.", FileName = s.FileName, Offset = s.Offset, Length = s.Length });
